...
def calc_time_e(inicio,chat,sms,bot,error_pr_find):
    fin = datetime.datetime.now()
    final = datetime.datetime(fin.year, fin.month, fin.day, fin.hour,fin.minute,fin.second)
    result = final-inicio
    bot.editMessageText(chat_id=chat,message_id=sms.message_id,parse_mode="HTML",text=error_pr_find+f'\n\n<b>🕰 TT : </b>{result}')
...
def animacion(anim,bot,chat,texto):
    msg=''
    for i in anim:
        msg+=i
        time.sleep(0.3)